...
It also demonstrates all supported control-plane component customizations:

- Retaining Kubernetes events for 30 minutes and explicitly setting the API server NodePort range.
- Running the horizontal pod autoscaler controller sync every 10 seconds.
- Using the scheduler's `MostAllocated` NodeResourcesFit strategy with equal CPU and memory weights.

The HPA controller configuration requires a Provisioned Control Plane tier and cannot be used with `standard`.
