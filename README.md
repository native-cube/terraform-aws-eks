...
- Optional Amazon EKS managed Argo CD capability
...
Enable the `argocd` object when this module should create an Amazon EKS managed Argo CD capability:
...
When enabled, the module can create the capability IAM role trusted by `capabilities.eks.amazonaws.com`, create the `aws_eks_capability` resource with type `ARGOCD`, configure IAM Identity Center authentication, and expose the managed Argo CD server URL. Attach managed policies or an inline policy to the capability role only when Argo CD needs access to supporting AWS services such as Secrets Manager, CodeConnections, or ECR.

If `argocd.network_access_vpce_ids` is empty, AWS exposes the default public managed Argo CD endpoint. Supplying VPC endpoint IDs makes the Argo CD server private-only through those endpoints.
...
| [aws_iam_role_policy.argocd_capability](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy) | resource |
| [aws_iam_role_policy_attachment.argocd_capability](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy_attachment) | resource |
...
| <a name="input_argocd"></a> [argocd](#input\_argocd) | Optional Amazon EKS Argo CD capability configuration. This creates the EKS managed Argo CD capability and, by default, a capability IAM role; it does not create Git repositories, Argo CD applications, or IAM Identity Center users/groups. | <pre>object({<br/>    capability_name           = optional(string, "argocd")<br/>    create_iam_role           = optional(bool, true)<br/>    delete_propagation_policy = optional(string, "RETAIN")<br/>    enabled                   = optional(bool, false)<br/>    iam_policy_arns           = optional(set(string), [])<br/>    iam_role_arn              = optional(string)<br/>    iam_role_name             = optional(string)<br/>    idc_instance_arn          = optional(string)<br/>    idc_region                = optional(string)<br/>    inline_policy_json        = optional(string)<br/>    namespace                 = optional(string)<br/>    network_access_vpce_ids   = optional(set(string), [])<br/>    rbac_role_mappings = optional(list(object({<br/>      role = string<br/>      identities = list(object({<br/>        id   = string<br/>        type = string<br/>      }))<br/>    })), [])<br/>  })</pre> | `{}` | no |
...
| <a name="output_argocd_capability_arn"></a> [argocd\_capability\_arn](#output\_argocd\_capability\_arn) | ARN of the Amazon EKS Argo CD capability when enabled. |
| <a name="output_argocd_capability_iam_role_arn"></a> [argocd\_capability\_iam\_role\_arn](#output\_argocd\_capability\_iam\_role\_arn) | IAM role ARN used by the Amazon EKS Argo CD capability when enabled. |
| <a name="output_argocd_capability_iam_role_name"></a> [argocd\_capability\_iam\_role\_name](#output\_argocd\_capability\_iam\_role\_name) | IAM role name used by the Amazon EKS Argo CD capability when enabled. |
| <a name="output_argocd_capability_name"></a> [argocd\_capability\_name](#output\_argocd\_capability\_name) | Name of the Amazon EKS Argo CD capability when enabled. |
| <a name="output_argocd_capability_version"></a> [argocd\_capability\_version](#output\_argocd\_capability\_version) | Version of the Amazon EKS Argo CD capability software when enabled. |
| <a name="output_argocd_idc_managed_application_arn"></a> [argocd\_idc\_managed\_application\_arn](#output\_argocd\_idc\_managed\_application\_arn) | IAM Identity Center managed application ARN created for the Amazon EKS Argo CD capability when enabled. |
| <a name="output_argocd_server_url"></a> [argocd\_server\_url](#output\_argocd\_server\_url) | Managed Argo CD server URL exposed by the Amazon EKS Argo CD capability when enabled. |
